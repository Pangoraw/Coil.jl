...
Base.cconvert(::Type{MlirDialectRegistry}, registry::DialectRegistry) = registry
Base.unsafe_convert(::Type{MlirDialectRegistry}, registry::DialectRegistry) = registry.registry
...
function DenseArrayAttribute(values::AbstractVector{Int64})
    Attribute(
...
function create_operation(
    ...
    GC.@preserve name loc begin
        ...
        if !isnothing(operands)
            API.mlirOperationStateAddOperands(state, length(operands), operands::Vector{Value})
        end
...
function Base.show(io::IO, operation::Operation)
    ...
    API.mlirOperationPrintWithFlags(operation, flags, c_print_callback, buf)
    API.mlirOpPrintingFlagsDestroy(flags)
    write(io, rstrip(String(take!(buf))))
end
...
function owner(op_operand::OpOperand)
    raw_op = LibMLIR.mlirOpOperandGetOwner(op_operand)
    Operation(raw_op, false)
end

operand_number(op_operand) = 1 + LibMLIR.mlirOpOperandGetOperandNumber(op_operand)
function next_use(op_operand)
