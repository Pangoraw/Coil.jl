...
function run!(pm::PassManager, module_)
    status = API.mlirPassManagerRunOnOp(pm, get_operation(module_))
...
@static if isdefined(API, :mlirCreateExternalPass)
    ...
    opname(::AbstractPass) = ""
    function pass_run(::P, op) where {P<:AbstractPass}
